...
export default function forgotPassword({ header }: any) {
  ...
  const forgotPasswordApiUrl =
    'https://fair-conduit-404516.uc.r.appspot.com/big-commerce/update-password'
  const checkSession = 'https://fair-conduit-404516.uc.r.appspot.com/big-commerce/check-session'
  const resetPasswordApiUrl = 'https://fair-conduit-404516.uc.r.appspot.com/email'
  ...
  const handleForgotPassword = async (e: React.SyntheticEvent<EventTarget>) => {
    ...
    try {
      ...
      await axios.put(forgotPasswordApiUrl, data).then(
        (response) => {
          setLoading(false)
